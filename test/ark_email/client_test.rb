...
class ArkEmailTest < Minitest::Test
  ...
  def test_client_default_request_default_retry_attempts
    stub_request(:post, "http://localhost/emails").to_return_json(status: 500, body: {})

    ark = ArkEmail::Client.new(base_url: "http://localhost", api_key: "My API Key")

    assert_raises(ArkEmail::Errors::InternalServerError) do
  ...
  def test_client_given_request_default_retry_attempts
    ...
    ark = ArkEmail::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 3)
  ...
  def test_client_retry_after_seconds
    stub_request(:post, "http://localhost/emails").to_return_json(
      status: 500,
      headers: {"retry-after" => "1.3"},
      body: {}
    )

    ark = ArkEmail::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 1)
  ...
  def test_client_redirect_307
    ...
    assert_raises(ArkEmail::Errors::APIConnectionError) do
      ark.emails.send_(
        from: "Acme <hello@acme.com>",
        subject: "Hello World",
        to: ["user@example.com"],
    ...
    assert_requested(:any, "http://localhost/redirected", times: ArkEmail::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_303
    ...
    assert_requested(:get, "http://localhost/redirected", times: ArkEmail::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_auth_strip_cross_origin
    ...
    assert_requested(:any, "https://example.com/redirected", times: ArkEmail::Client::MAX_REDIRECTS) do
      headers = _1.headers.keys.map(&:downcase)
  ...
  def test_default_headers
    stub_request(:post, "http://localhost/emails").to_return_json(status: 200, body: {})
